...
![skillfrisk проверяет вредоносный скилл и чистый](https://raw.githubusercontent.com/Topicspot/skillfrisk/main/assets/demo.gif)
...
## Гейт обновлений: `skillfrisk diff`

Менеджеры скиллов обновляют по хешу папки и не показывают, что изменилось в инструкциях.
`skillfrisk diff старая/ новая/` сравнивает две локальные версии: новые находки, изменение
прав (`allowed-tools`, shell-команды, сетевые хосты) и вердикт с кодом возврата `2` при
новом риске. `--fail-on any-change` валит гейт при любом росте привилегий. Находки
сопоставляются семантически, поэтому сдвиг текста не даёт ложных «новых» срабатываний.
