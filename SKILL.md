...
# skillscan — vet skills and MCP servers before trusting them

Third-party skills and MCP servers are executable instructions for your agent.
Scan them before installing. skillscan statically checks SKILL.md files, bundled
...
```bash
uvx --from "git+https://github.com/Topicspot/skillscan" skillscan scan path/to/skill-or-mcp
```

Machine-readable output for your own decision-making:

```bash
uvx --from "git+https://github.com/Topicspot/skillscan" skillscan scan path/to/skill --json
```
...
3. Run skillscan on that directory.
